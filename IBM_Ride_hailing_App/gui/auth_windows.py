...
class RegisterWindow(BaseWindow):
    ...
    def setup_ui(self):
        """Setup the registration UI with a scrollable frame"""
        # Title
        ...
        # Registration form inside the scrollable frame
        ...
        rider_radio = tk.Radiobutton(type_frame, text="Rider", variable=self.user_type, 
                                   value="rider", bg='#f0f0f0', command=self.on_user_type_change)
        driver_radio = tk.Radiobutton(type_frame, text="Driver", variable=self.user_type, 
                                    value="driver", bg='#f0f0f0', command=self.on_user_type_change)
        rider_radio.pack(anchor='w')
        driver_radio.pack(anchor='w')
        
        # License number field (for drivers)
        self.license_frame, self.license_entry = self.create_entry_field("Driver License Number (required for drivers)", form_frame)
        self.license_frame.pack(fill=tk.X, pady=10)
        # Set initial state: disable for rider, enable for driver
    ...
    def on_user_type_change(self, *args):
        """Enable/disable license field based on user type"""
        if self.user_type.get() == "driver":
            self.license_entry.config(state='normal')
        else:
            self.license_entry.delete(0, tk.END)
